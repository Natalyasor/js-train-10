...
function customAssign(...objects) {
  ...
  if(objects.length < 2) {
    return "Помилка: Має бути передано принаймі два обєкти."
  }
...
function customObjectFromEntries(entries) {
  ...
  const entries1 = entries.map(([key, value]) => {
  ...
  const result = Object.fromEntries(entries1);
...
function checkProperty(obj, prop) {
  ...
  const obj1 = obj.hasOwnProperty(prop);
...
if(!Array.isArray(arr)) {
  ...
  arr.filter((obj) => obj.hasOwnProperty(key));
...
const obj1 = Object.preventExtensions(obj);
  ...
  const obj2 = Object.isExtensible(obj1);
...
function getObjectValuesSum(obj) {
  ...
  const sum = obj1.reduce(function(currentSum, currentNumber) {
